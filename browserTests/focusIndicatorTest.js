  ...
  test('Input fields have correct focus indicators', async (t) => {
    ...
    for(let i = 0; i < inputFieldsCount; i++) {
      ...
      if (disabled === undefined || disabled === false) {
        ...
        const contrast = getContrast(parentBackground, await focusedElement.getStyleProperty('box-shadow'));
        ...
        await t
          .expect(contrast).gte(3, `Input contrast ratio between focus border and background is not enough`)
          .expect(contrastWithElement).gte(3, `Input contrast ratio between focus border and background is not enough`)
     }
    }
  })

  test('Buttons have correct focus indicator', async (t) => {
