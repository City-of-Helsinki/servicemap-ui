...
export const fetchDistricts =
  (selected, single, period) => async (dispatch) => {
    const categories = single
      ? selected
      : dataStructure
          .map((obj) => obj.districts.map((item) => item.id))
          .flat()
          .join(',');
...
export const fetchParkingAreaGeometry = (areaId) => async (dispatch) => {
...
export const fetchAccessibleStreetParking = () =>
  fetchParkingUnits('vantaa-2207');

export const handleOpenItems = (id) => (dispatch, getState) => {
...
export const handleOpenGeographicalCategory = (id) => (dispatch, getState) => {
  const { openItems } = getState().districts;
  const stateCategory = openItems.find((item) =>
