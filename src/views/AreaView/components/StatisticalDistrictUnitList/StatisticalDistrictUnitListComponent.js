      ...
      <StyledUnitListArea>
        ...
        >
          {filterValue &&
            ...
            formatMessage(
              {
                id: 'area.statisticalDistrict.service.filter.aria.notification',
              },
              { filterValue }
            )}
        </Typography>
        <List disablePadding>
          {filteredServiceList.map((service) => {
            const units = statisticalDistrictUnits.filter((u) =>
              u?.services?.some((s) => s.id === service.id)
            );
            const disableUnitAccordion =
              !selectedServices[service.id] || units.length === 0;
            const serviceName = uppercaseFirst(getLocaleText(service.name));
            const unitCount = selectedServices[service.id]
              ? `(${units.length})`
              : '';
            const titleText = `${serviceName} ${unitCount}`;
            return (
              <StyledListItem
