...
const GeographicalUnitList = ({ getLocaleText, handleOpen, classes }) => {
  ...
  const filteredSubdistrictUnits = useSelector(state => getSubdistrictServices(state));
  const [initialOpenItems] = useState(areaViewState?.openItems || []);
  // Get initial state only once to prevent useless re-rendering
  const initialCheckedItems = useStore()?.getState()?.districts?.selectedDistrictServices;
