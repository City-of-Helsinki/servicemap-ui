...
const expandSearch = async (searchQuery, getLocaleText, cities, locale, amount, signal) => {
  const listLength = amount || 10;
  const pageSize = 200;

  const results = await fetch(`https://api.hel.fi/servicemap/v2/search/?q=${searchQuery}&municipality=${cities}&language=${locale}&only=unit.name,unit.services&include=unit.services&page_size=${pageSize}`, {
    signal,
  })
    ...
    .then(async (suggestions) => {
      ...
      const data = await Promise.all(suggestions.map(word => fetch(`https://api.hel.fi/servicemap/v2/search/?q=${word}&municipality=${cities}&language=${locale}&only=unit.name&page_size=0&type=unit`)
        .then(response => response.json())));
