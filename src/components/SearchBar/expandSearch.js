const expandSearch = async (item, getLocaleText) => {
  const listLength = 5;
  // const pageSize = 200;

  const results = await fetch(`https://api.hel.fi/servicemap/v2/search/?q=${item.query}&only=unit.name,unit.services&include=unit.services&page_size=${item.count}`)
    .then(response => response.json())
    .then((data) => {
      // Form list of suggested search prases
      const suggestions = [];
      for (let i = 0; suggestions.length < listLength && i < item.count; i += 1) {
        ...
        if (result.object_type === 'unit') {
          result.services.forEach((service) => {
            ...
            if (!item.query.toUpperCase().includes(getLocaleText(service.name).toUpperCase())) {
    ...
    .then(async (suggestions) => {
      ...
      data.forEach((res, i) => {
        ...
        if (res.count > -1) { // this should be changed to > 0 when the backend returns correct results
          /* This calculates if the suggestion results have 2 or more unique services (if 2 or more, "tarkenna" button can be displayed)
           this can be removed once we get number from backend */
          const services = [];
          ...
          if (services.length < 2) {
            ...
            /* If only one result found with a a suggestion, add the whole original name of the result,
