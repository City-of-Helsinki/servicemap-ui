  ...
  switch (page) {
    ...
    case 'address': {
      ...
      if (addressAdminDistricts?.length) {
        const neighborhood = addressAdminDistricts.find(
          (obj) => obj.type === 'neighborhood'
        );
        const postcode = addressAdminDistricts.find(
          (obj) => obj.type === 'postcode_area'
        );
        if (neighborhood) {
          description += `${intl.formatMessage({
            id: 'area.list.neighborhood',
          })}: ${getLocaleText(neighborhood.name)}. `;
        }
        if (postcode) {
    ...
    case 'unit': {
      ...
      const openingHoursData = unit.connections?.find(
        (obj) => obj.section_type === 'OPENING_HOURS'
      );
      ...
      if (unit.accessibility_viewpoints) {
        description += intl.formatMessage({
          id: 'unit.seo.description.accessibility',
        });
      } else {
